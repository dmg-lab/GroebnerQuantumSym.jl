...
+sum(u[2,s]*u[4,t]*inj(3,j,r,u=u)*u[w,2] for j in (3:n))
+sum(u[2,s]*u[4,t]*inj(3,3,r,u=u)*u[w,e] for e in (4:n))
+sum(u[2,s]*u[4,t]*inj(3,a,r,u=u)*u[w,e] for a in (4:n) for e in (4:n) if a!=e)
#-sum(u[2,s]*inj(4,t,3,u=u)*u[r,c]*u[w,e] for c in (2:n) for e in (3:n))
+sum(u[2,s]*u[4,t]*u[3,t]*u[r,d]*row_sum(w,u) for d in (3:n))
-sum(u[2,s]*u[j,t]*bg(2,3,r,w,u=u) for j in (5:n))
#+sum(u[3,s]*col_sum(t,u)*u[3,i]*u[r,j]*u[w,k] for i in (2:n) for j in (4:n) for k in (3:n))
;len=300
))
