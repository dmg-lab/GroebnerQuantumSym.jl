#Reduction of Bg8243
function bg8243_data(n::Int)

G1 = g1_named(n)
u = magic_unitary(n)
to_be_reduced() = bg(8,2,4,3; u=u)
free_vars = ()
...
reduction(n::Int) = begin (
-sum([E1["bg2_2$(k)3"] for k in 4:n])+sum([E1["bg8_2$(k)3"] for k in 5:n])
-u[3,2]*E1["cs4"]*u[3,3] +u[3,2]*E1["rwel43"]+E1["wel324"]*u[3,3]
+help1(3,2)-u[3,2]*E1["rwel43"]+E1["ucsu32433"]

-sum([E1["wel32$g"]*u[3,3] for g in 5:n])


+sum([help1(g,2) for g in 4:n])
...
-sum([E1["wel$(g)23"]+(n-2)*sum([E1["wel$(g)2$i"] for i in 4:n]) for g in 4:n])
-(n-4)*sum([E1["wel$(g)3$i"] for g in 4:n for i in 2:n if i != 3])
+sum([E1["wel$(g)$(h)3"]-(n-4)*sum([E1["wel$(g)$h$i"] for i in 2:n if i != 3 && i != h]) for g in 4:n for h in 4:n])


-(n-2)*sum([E1["cs2"]*u[3,i] for i in 4:n])

  +sum([((n-2)*sum([u[k,2]*E1["cs$i"] for i in 4:n]) -(n-3)*u[k,2]*E1["cs3"]) for k in 3:n])

+sum([((2*n-6)*sum([u[g,h]*E1["cs$i"] for g in 3:n for i in 2:n if i != 3])-(n-3)*sum([E1["cs$h"]*u[3,i] for i in 2:n if i != h && i != 3])) for h in 3:n])

+sum([E1["cs$g"]*u[3,3] for g in 4:n])
  -sum([(u[h,g]*E1["cs3"] +(n-4)*u[h,g]*E1["cs$g"]) for h in 3:n for g in 4:n])

-(n^2-5*n+6)*E1["cs2"]
-(n^2-5*n+7)*sum([E1["cs$i"] for i in 4:n])
+(n-3)*E1["cs3"]

+(n-2)*sum([E1["rs2"]*u[i,3] for i in 2:n if i != 3 && i != 2])
+(n-3)* sum([E1["rs3"]*u[i,3] for i in 2:n if i != 3 ])
+sum([((n-3)*sum([E1["rs$g"]*u[i,3] for i in 2:n if i != 3 && i != g]) -E1["rs$g"]*u[3,3]) for g in 4:n])

+(n-3)*u[2,3]*E1["rs3"]
-(n-3)*sum([u[2,3]*E1["rs$i"] for i in 4:n])
+(n-3)*sum([u[2,i]*E1["rs3"] for i in 4:n])

-sum(u[2,3]*E1["rs$i"] for i in 4:n)

-(2*n-6)*sum(u[h,g]*E1["rs$i"] for h in 3:n for g in 3:n for i in 2:n if h != i && i != 3)

-(n-2)*sum([u[g,h]*E1["rs$g"] for h in 3:n for g in 4:n])
-(n-2)*sum([u[2,h]*E1["rs$g"] for h in 4:n for g in 4:n])
+sum([u[g,h]*E1["rs3"] for g in 4:n for h in 3:n])

+(n^2-5*n+6)*E1["rs2"]
+(n^2-5*n+7)*sum([E1["rs$i"] for i in 4:n])
-(n-3)*E1["rs3"]
...
-(n-2)*sum([E1["rinj2$i"] for i in 4:n])
-(n-3)*sum([E1["rinj$g$i"] for g in 3:n for i in 2:n if i != g])
+(n-2)*sum([E1["rinj$(i)3"] for i in 4:n])
...
+sum([(E1["inj2$(h)3"]
+2*sum([E1["inj2$(h)$k"] for k in 4:n])
+sum([(n-4)*sum([E1["inj$(g)$(h)$k"] for k in 2:n if k != 3 && k != g]) for g in 2:n ]) 
-sum([E1["inj$(g)$(h)3"] for g in 4:n])) for h in 4:n])

-sum([u[h,j]*E1["rwel$k$i"]  for j in 3:n for h in 3:n for k in 2:n for i in 2:n if k != j && k != i && i != 3]) 

+(n-2)*sum([E1["rwel2$i"] for i in 2:n if 2 != i && i != 3])
+(n-3)*sum([E1["rwel$k$i"] for k in 3:n for i in 2:n if k != i && i != 3])
- sum([E1["rwel$(h)3"] for h in 4:n]))
end
return to_be_reduced, free_vars, reduction
end

@testset "bg8_243" verbose=true begin
 println("Testing examples of bg8_243...")

 @testset "n=5:10" begin
   for n in 5:10
     to_be_reduced, free_vars, reduction = bg8243_data(n)
     @test to_be_reduced()+reduction(n) == 0
   end
 end
end
